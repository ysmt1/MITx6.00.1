...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(alpha)):
            shift_dict[alpha[i]] = alpha2[i+shift]
            shift_dict[alpha[i].upper()] = alpha2[i+shift].upper()
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            tempWord = Message.apply_shift(self, shift)
            tempWordList = tempWord.split(' ')
            
            for word in tempWordList:
                if is_word(word_list, word):
                    wordCount += 1
                    
            if wordCount > tempWordCount:
                bestShift = shift
                decryptedMessage = tempWord
                tempWordCount = wordCount
